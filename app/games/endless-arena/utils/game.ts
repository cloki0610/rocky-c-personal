...
export const rangedDesc = (c: RangedAttack) =>
  c.mode === "stun"
    ? `stun ${c.duration / 1000}s · ${c.range}px`
    : `${c.dmg} dmg · ${c.range}px`;
...
export const stagePlan = (stage: number): StagePlan => {
  ...
  if (cycle === 1)
    return {
      minions: Math.min(16, 8 + loop * 2),
      buffed: Math.min(4, loop),
      boss: false,
    };
  if (cycle === 2)
    return {
      minions: Math.min(18, 11 + loop * 2),
      buffed: Math.min(5, 1 + loop),
      boss: false,
    };
  return {
    minions: Math.min(10, 2 + loop * 2),
    buffed: Math.min(3, loop),
    boss: true,
  };
};
...
export const center = (e: SizedEntity): Point => ({
  x: e.x + e.size / 2,
  y: e.y + e.size / 2,
});
...
export const spawnStage = (stage: number, px: number): StageEnemies => {
  ...
  if (plan.boss)
    boss = makeEnemy("boss", WORLD_W / 2 - TYPES.boss.size / 2, 60, sc);
...
export const fire = (w: World, conf: Attack, now: number) => {
  ...
  for (const t of targets) {
    if (dist(pc, center(t)) <= conf.range + t.size / 2) {
      ...
      if (t.hp <= 0) {
        t.hp = 0;
        puff(w, t.x, t.y, t.size, TYPES[t.type].color);
      }
    }
  }
};
  ...
  if (running) {
    ...
    w.bullets = w.bullets.filter((b) => {
      ...
      for (let s = 0; s < subs; s++) {
        ...
        b.y += b.dy * inc;
        b.travelled += inc;
        if (b.travelled >= b.max) {
          puff(w, b.x, b.y, b.size, b.color);
          return false;
        }
    ...
    for (const e of all) {
      ...
      if (d > e.reach - 6) {
        e.x = clamp(
          e.x + ((pc.x - ec.x) / d) * e.speed * dt,
          0,
          WORLD_W - e.size,
        );
        e.y = clamp(
          e.y + ((pc.y - ec.y) / d) * e.speed * dt,
          0,
          WORLD_H - e.size,
        );
      }
